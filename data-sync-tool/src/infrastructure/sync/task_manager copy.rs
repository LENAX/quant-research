...
pub enum SyncTaskQueueValue<'a> {
    Task(Option<Arc<Mutex<SyncTask<'a>>>>),
...
pub struct SyncTaskQueue<'a, T: RateLimiter> {
    tasks: Mutex<VecDeque<Arc<Mutex<SyncTask<'a>>>>>,
    rate_limiter: Option<T>,
}

impl<'a, T: RateLimiter> SyncTaskQueue<'a, T> {
    pub fn new(
        tasks: Vec<Arc<Mutex<SyncTask<'a>>>>,
        rate_limiter: Option<T>,
    ) -> SyncTaskQueue<'a, T> {
    ...
    pub async fn pop_front(&'a mut self) -> Result<SyncTaskQueueValue, TimerError> {
    ...
    pub async fn push_back(&'a mut self, task: Arc<Mutex<SyncTask<'a>>>) {
        let mut q_lock = self.tasks.lock().await;
        q_lock.push_back(task);
        return ();
    }

    pub async fn push_front(&'a mut self, task: Arc<Mutex<SyncTask<'a>>>) {
        let mut q_lock = self.tasks.lock().await;
...
pub struct TaskManager<T: RateLimiter> {
    queues: HashMap<DatasetId, Arc<Mutex<SyncTaskQueue<'static, T>>>>,
    task_channel: Sender<Arc<Mutex<SyncTask<'static>>>>,
    error_message_channel: Sender<TaskManagerError>,
    failed_task_receiver: Receiver<(DatasetId, Arc<Mutex<SyncTask<'static>>>)>,
}

impl<T: RateLimiter> TaskManager<T> {
    pub fn new(
        task_queues: Vec<(DatasetId, Arc<Mutex<SyncTaskQueue<'static, T>>>)>,
        task_channel: Sender<Arc<Mutex<SyncTask<'static>>>>,
        error_message_channel: Sender<TaskManagerError>,
        failed_task_receiver: Receiver<(DatasetId, Arc<Mutex<SyncTask<'static>>>)>,
    ) -> TaskManager<T> {
    ...
    pub async fn start(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        loop {
            let queues: Vec<Arc<Mutex<SyncTaskQueue<'static, T>>>> = self.queues.values().cloned().collect();
            for queue in queues {

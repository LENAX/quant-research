...
use sea_orm::{DatabaseConnection, TransactionTrait, Set, DeleteResult};
use uuid::Uuid;
...
use super::sync_plan_do::{ActiveModel, self, Column};
use sea_orm::prelude::*;
use anyhow::Result;
...
impl Repository<SyncPlan, SyncPlanQueryDTO, SyncPlanUpdateDTO> for SyncPlanRepositoryImpl {
    ...
    async fn add_all(&self, entities: Vec<SyncPlan>) -> Result<Uuid> {
        ...
        let res = sync_plan_do::Entity::insert_many(models).exec(&*self.db).await?;
    ...
    async fn find_many(&self, query: SyncPlanQueryDTO, page_size: usize, page_number: usize) -> Result<Vec<SyncPlan>> {
        let mut select = sync_plan_do::Entity::find();
        ...
        let result_page = paginator.fetch_page((page_number - 1).try_into().unwrap()).await?; // Page number is usually 1-indexed in user interfaces
    ...
    async fn update_one(&self, entity: SyncPlanUpdateDTO) -> Result<()> {
        ...
        if let Some(trigger_time) = entity.trigger_time() {
            update = update.col_expr(sync_plan_do::Column::TriggerTime, Expr::value(*trigger_time));
        }
        if let Some(frequency) = entity.frequency() {
            // Assuming frequency is converted to a string for the database
            update = update.col_expr(sync_plan_do::Column::Frequency, Expr::value(frequency.to_string()));
        }
        ...
        if let Some(sync_config) = entity.sync_config() {
            ...
            update = update.col_expr(sync_plan_do::Column::SyncConfig, Expr::value(serde_json::to_string(&sync_config)?));
        }
        // ... other fields ...
    
        // Apply the update statement to the database
        update.filter(sync_plan_do::Column::Id.eq(*plan_id)).exec(&*self.db).await?;
    ...
    async fn update_many(&self, entities: Vec<SyncPlanUpdateDTO>) -> Result<()> {
        ...
        for entity in entities.iter() {
            let mut active_model: sync_plan_do::ActiveModel = <sync_plan_do::ActiveModel as sea_orm::ActiveModelTrait>::default();
            ...
            if res.is_err() {
                ...
                let _ = txn.rollback().await.map_err(|err| Box::new(anyhow::anyhow!(err)));
                return Err(Box::new(res.err().unwrap()).into());
            }
        }

        // If all updates succeed, commit the transaction
        let _ = txn.commit().await.map_err(|err| Box::new(anyhow::anyhow!(err)));
        Ok(())
    }
    ...
    async fn delete_one(&self, query: SyncPlanQueryDTO) -> Result<()> {
        ...
        let _ = delete_stmt.exec(&*self.db).await.map_err(|err| Box::new(anyhow::anyhow!(err)));
        Ok(())
    ...
    async fn delete_many(&self, queries: Vec<SyncPlanQueryDTO>) -> Result<()> {
        ...
        for query in queries {
            ...
            let _ = delete_stmt.exec(&*self.db).await.map_err(|e| Box::new(e) as Box<dyn Error>);
        }
...
mod tests {
    use super::*;
    use log::info;
    use sea_orm::{Database, MockDatabase, EntityTrait, Set, DbBackend};
    use uuid::Uuid;


    // Function to set up an in-memory database or mock
    async fn setup_database() -> Arc<DatabaseConnection> {
